...
class BYOLplus(nn.Module):
    def __init__(self, args):
        super(BYOLplus, self).__init__()
        self.args = args
    ...
    def build_model(self, x1, x2, lambd):
        ...
        oz1, tz1, p1 = self.encoder(x1)
        oz2, tz2, p2 = self.encoder(x2)
        z1_norm = F.normalize(oz1, dim=-1)
        z2_norm = F.normalize(oz2, dim=-1)

        with torch.no_grad():
            self.encoder._momentum_update_target_encoder()
        alignment_loss = self.encoder.calc_alignment_loss(
            tz1.detach(), p1, tz2.detach(), p2
        )
        wasserstein_loss = self.encoder.calc_wasserstein_loss(z1_norm, z2_norm)
        total_loss = alignment_loss + lambd * wasserstein_loss
    ...
    def train_model(self, args):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            total_loss, alignment_loss, wasserstein_loss, total_num = 0.0, 0.0, 0.0, 0
            iter_count = 0
            ...
            for step, ((x1, x2), _) in enumerate(
                ...
                alignment_loss += print_pack.alignment_loss.item() * batch_size
                wasserstein_loss += print_pack.wasserstein_loss.item() * batch_size
                if iter_count % 10 == 0:
                    print(
                        train_loss.pprint(
                            window=30,
                            prefix="Train Epoch: [{}/{}] Iters:[{}/{}]".format(
                                epoch,
                                args.epochs,
                                iter_count,
    ...
    def calc_uniformity(self, args):
        ...
        with torch.no_grad():
            for (data, _), target in tqdm(
                ...
                _, _, _, z_norm = self.encoder.obtain_representation(
    ...
    def calc_alignment(self, args):
        ...
        with torch.no_grad():
            for (x1, x2), target in tqdm(
                ...
                _, _, z1, z1_norm = self.encoder.obtain_representation(
                    x1.cuda(non_blocking=True)
                )
                _, _, z2, z2_norm = self.encoder.obtain_representation(
                    x2.cuda(non_blocking=True)
                )
    ...
    def dimensional_analysis(self, args):
        ...
        with torch.no_grad():
            ...
            print(args.save_name_pre)
            plt.plot(spectrum)
            plt.savefig("saver/" + args.save_name_pre + ".png")
            plt.close()
            print("spectrum:", spectrum)
            savemat("saver/" + args.save_name_pre  + ".mat", {"spectrum": spectrum})
    ...
    def calc_knn_accuracy(self, epoch, args):
        ...
        with torch.no_grad():
            for (data, _), target in tqdm(
                ...
                _, feature, _, z_norm = self.encoder.obtain_representation(
                    data.cuda(non_blocking=True)
                )
            ...
            for (data, _), target in test_bar:
                ...
                _, feature, _, _ = self.encoder.obtain_representation(data)
    ...
    def linear_train_val(self, args, epoch, optimizer, criterion, is_train=True):
        ...
        with torch.enable_grad() if is_train else torch.no_grad():
            for data, target in data_bar:
                ...
                feature, _, _, _ = self.encoder.obtain_representation(data)
