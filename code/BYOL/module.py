...
class PartialEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.projector = nn.Sequential(
            nn.Linear(512, args.hidden_dim, bias=False),
...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        self.predictor = nn.Sequential(
            nn.Linear(args.prej_dim, args.predictor_dim, bias=False),
            nn.BatchNorm1d(args.predictor_dim),
            nn.ReLU(inplace=True),
    ...
    def _momentum_update_target_encoder(self):
        ...
        for param_o, param_t in zip(
            ...
            param_t.data = param_t.data * self.args.momentum + param_o.data * (
    ...
    def calc_alignment_loss(self, z1, p1, z2, p2):
        alignment_loss = 2.0 * (1.0 - self.criterion(z1, p2).mean()) + 2.0 * (
            1.0 - self.criterion(z2, p1).mean()
        )
    ...
    def calc_wasserstein_loss(self, z1, z2):
        ...
        # S, Q = torch.eig(covariance, eigenvectors=True) # need to change this, no longer supported
        S, Q = torch.linalg.eig(covariance)  # -- julie changed this
        Q = Q.real  # -- julie changed this
        S = torch.abs(S.real)  # -- julie changed this
