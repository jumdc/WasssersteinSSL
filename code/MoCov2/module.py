...
class Encoder(nn.Module):
    ...
    def calc_contrastive_loss(self, q_norm, k_norm, negative_embed):
        ...
        pos = torch.exp(
            torch.sum(q_norm * k_norm, dim=-1) / self.args.temperature
        )  ##[batch_size]
        ...
        neg = torch.exp(
            torch.bmm(q_norm.unsqueeze(1), negative_embed).squeeze(1)
    ...
    def calc_wasserstein_loss(self, z1, z2):
        ...
        mean = z.mean(0)
        covariance = (
            torch.mm((z - z_center).t(), z - z_center) / N + 1e-4 * torch.eye(D).cuda()
        )
